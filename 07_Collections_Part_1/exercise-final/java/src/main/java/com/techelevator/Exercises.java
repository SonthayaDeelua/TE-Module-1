...
public class Exercises {
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		for (int i = 0; i < intArray.length; i++) {
			...
			newList.add(new Double(intArray[i] / 2));
		}
		return newList;
	}
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		...
		for(int i = 0; i < integerArray.length; i++) {
			if((integerArray[i]%15) == 0) {
				...
				newString.add(integerArray[i].toString());
			}
		}
		return newString;
	}
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		...
		Queue<Integer> mediemQueue = new LinkedList<>();
		Queue<Integer> largeQueue = new LinkedList<>();
		
		for(Integer number : seatNumberList) {
			if(number > 0 && number <= 10) {
				smallQueue.add(number);
			}else if(number > 10 && number <= 20) {
				mediemQueue.add(number);
		...
		List <Integer> newList = new ArrayList<>();
		newList.addAll(smallQueue);
		newList.addAll(mediemQueue);
		newList.addAll(largeQueue);
