...
public class Exercises {
	 ...
	 * → "HiByeByeHi" makeAbba("Yo", "Alice") → "YoAliceAliceYo"
	 ...
	 * going up to but not including index j. makeOutWord("<<>>", "Yay") →
	 * "<<Yay>>" makeOutWord("<<>>", "WooHoo") → "<<WooHoo>>" makeOutWord("[[]]",
	 * "word") → "[[word]]"
	 */
	...
	public String extraEnd(String str) {
		...
		if(str.length() >= 2) {
			newString =  newString + str.substring(str.length()-2) +  str.substring(str.length()-2) + 
	 ...
	 * yields "Woo". firstHalf("WooHoo") → "Woo" firstHalf("HelloThere") →
	 * "Hello" firstHalf("abcdef") → "abc"
	 */
	...
	public String right2(String str) {
		String newString = "";
		if (str.length() >= 2) {
			newString = newString + str.substring(str.length() - 2) + str.substring(0, str.length() - 2);
		}
		return newString;
	...
	public String middleTwo(String str) {
		...
		if (str.length() >= 2) {
			newString = newString + str.substring(middleIndex - 1, middleIndex + 1);
		}
		return newString;
	 ...
	 * 2) → "Helo" nTwice("Chocolate", 3) → "Choate" nTwice("Chocolate", 1) →
	 * "Ce"
	 */
	public String nTwice(String str, int n) {
		...
		if (n >= 0 && str.length() >= 2) {
			newString = newString + str.substring(0, n) + str.substring(str.length() - n);
	...
	public String middleThree(String str) {
		...
		if (length >= 3) {
			newString = newString + str.substring((middleIndex - 1), (middleIndex + 2));
	...
	public boolean hasBad(String str) {
		...
		if (strLenght >= 3 && str.indexOf("bad") == 0) {
			return true;
		} else if (strLenght >= 3 && str.indexOf("bad") == 1) {
			return true;
		}
	...
	public String frontTimes(String str, int n) {
		...
		if (str.length() > 3) {
			for (int i = 1; i <= n; i++) {
				front3 = front3 + str.substring(0, 3);
			}
		} else {
			for (int i = 1; i <= n; i++) {
				front3 = front3 + str;
	...
	public int last2(String str) {
		...
		for (int i = 0; i < str.length() - 1; i++) {
			...
			if ((i + 1) < str.length() - 1) {
				...
				if (str.substring(i, i + 2).contains(str.substring(str.length() - 2, str.length()))) {
	...
	public String stringX(String str) {
		...
		if (str.length() < 3) {
			...
			for (int i = 1; i < str.length() - 1; i++) {
				...
				if (str.charAt(i) != 'x') {
					insideString = insideString + str.charAt(i);
				}

				newString = str.charAt(0) + insideString + str.charAt(str.length() - 1);
	...
	// i+=4 make it can start at the next position
